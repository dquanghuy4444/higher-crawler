"""
YTDown Crawler using CloakBrowser (stealth Chromium).
...
DEFAULT_PROFILE_DIR = str(Path.cwd() / ".browser-profiles" / "ytdown-cloakbrowser")


def log_info(message, details=None):
    entry = {"level": "info", "source": "ytdown-cloak", "message": message}
    if details:
...
def log_warn(message, details=None):
    entry = {"level": "warn", "source": "ytdown-cloak", "message": message}
    if details:
        entry["details"] = details
    print(json.dumps(entry), file=sys.stderr)
...
def log_error(message, details=None):
    entry = {"level": "error", "source": "ytdown-cloak", "message": message}
    if details:
        entry["details"] = details
    print(json.dumps(entry), file=sys.stderr)
...
async def wait_for_app_input(page, timeout_ms, profile_dir):
    ...
    while time.time() < deadline:
        ...
        if is_cloudflare_page(state.get("text", ""), state.get("title", "")):
            log_warn("Cloudflare still visible. Complete verification in the CloakBrowser window.", {
...
async def crawl(input_data):
    ...
    log_info("Starting YTDown CloakBrowser bot.", {
    ...
    log_step("Launching CloakBrowser.", {"headless": not browser_visible, "profile_dir": profile_dir})

    # CloakBrowser: launch_persistent_context_async gives us a BrowserContext
    # with persistent cookies/localStorage + stealth Chromium
    ...
    try:
        ...
        if is_cloudflare_page(state.get("text", ""), state.get("title", "")):
            log_info("Cloudflare challenge detected. Waiting for auto-bypass...")
